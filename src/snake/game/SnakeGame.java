...
public class SnakeGame extends Game implements KeyListener {
    ...
    public void takeTurn() {
        for(Snake s :snake_liste){ 
            ...
            if(s.is_sick == false){
                for (Item item : new ArrayList<>(item_liste)) {//utilisation d'une copie pour éviter les problèmes de suppression
                    ...
                    if (item.itemType == ItemType.APPLE &&
                        ...
                        if (Math.random() < pitem) {
                            ...
                            if (bonusPosition != null) {
                                ItemType[] typesBonus = {ItemType.BOX, ItemType.SICK_BALL, ItemType.INVINCIBILITY_BALL};
    ...
    public void agent_mort(){
        ...
        for (Snake s : snake_liste) {
            ...
            for (Snake otherSnake : snake_liste) {
                if (otherSnake == s) continue;//ignore le serpent lui-même
